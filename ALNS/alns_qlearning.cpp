...
void ALNS_QLearning::initOps() {
    ...
    // NUEVO: 3 Estados basados en el estancamiento de la busqueda
    // 0: Mejorando (0-50 iteraciones sin mejora)
    // 1: Estancado (51-150 iteraciones sin mejora)
    // 2: Atrapado (>150 iteraciones sin mejora)
    num_states = 3; 

    // NUEVO: Tablas desacopladas para reducir el espacio de exploracion
...
// NUEVO: Funcion para calcular las probabilidades Softmax (reemplaza a epsilon-greedy)
std::vector<double> ALNS_QLearning::getSoftmaxProbabilities(const std::vector<double>& q_values, double tau) {
    ...
    double max_q = *std::max_element(q_values.begin(), q_values.end()); // Para estabilidad numerica
    double sum = 0.0;
...
Solution ALNS_QLearning::solve(int max_iters) {
    ...
    int current_state = 0; 
    int iters_without_improvement = 0; // Contador para definir el estado
    double tau = 1.0; // Temperatura de Boltzmann (puede decaer o ser constante)
    ...
    for (int iter = 0; iter < max_iters; ++iter) {
        ...
        // 1. Q-Learning: Calcular probabilidades y seleccionar operadores (Softmax)
        ...
        // 4. Actualizar contadores y determinar el Siguiente Estado
        if (global_improved) {
        ...
        // 5. Q-Learning: Ecuaciones de Bellman Desacopladas
        ...
        data.score = reward;  // <-- NUEVO: En este diseño, el score térmico coincide con la recompensa base
        data.reward = reward; // <-- Mantenemos el reward para futuras lógicas del RL
...
void ALNS_QLearning::exportMetrics(const std::string& filename) {
    ...
    // NUEVO: Agregamos 'score' a la cabecera
    file << "iter,best_veh,best_dist,curr_veh,curr_dist,d_op,r_op,score,reward,temp";
    ...
    for (const auto& data : history) {
        file << data.iter << ","
             ...
             << data.d_idx << ","
             << data.r_idx << ","
             << data.score << ","  // <-- NUEVO: Imprimimos el score
             << data.reward << ","
             << data.temp;
